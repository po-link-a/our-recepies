...
Строгие правила:
1. Сначала точно прочитай всё, что написано на фото. Ничего не выдумывай и не дополняй.
2. Затем переведи рецепт на РУССКИЙ язык: title, ingredients (name, unit, note) и directions — всё по-русски, даже если на фото украинский или французский.
3. Перевод дословный. Не «улучшай» текст, не добавляй и не убирай шаги и детали. Меры записывай привычными русскими обозначениями («г», «мл», «шт», «ст. ложка», «ч. ложка», «стакан»), НЕ пересчитывая сами величины.
4. language — язык ОРИГИНАЛА на фото: RU, UK или FR. Это поле не о переводе, а о том, на каком языке была вырезка.
5. Не добавляй никаких приписок об источнике («записано со слов бабушки», «семейный рецепт» и т.п.), если этого нет на фото.
6. ingredients: name — название, amount — число, unit — ТОЛЬКО короткая единица измерения («г», «мл», «шт», «ст. ложка», «ч. ложка», «стакан»). В unit не должно быть цифр, пояснений и второй меры. Если в оригинале указано две меры («30 г / 2 ст. ложки»), поставь первую в amount + unit, а вторую целиком в note. Если количество не указано («по вкусу»), amount и unit не заполняй, а напиши это в note.
7. directions: массив шагов на русском. Если в оригинале сплошной текст — раздели на логические шаги, не меняя смысла.
8. sourceNote: заполняй, только если на фото видно название издания, рубрики или подпись. Название издания НЕ переводи — оставь как на фото («Cuisine et Vins de France»). Иначе оставь пустым.
9. category — ближайшая из списка. title — русское название блюда; если названия на фото нет, коротко опиши блюдо по его составу.
10. Время указывай, только если оно есть на фото.
11. Если часть текста обрезана, нечитаема или явно продолжается за краем фото — поставь isIncomplete: true и опиши проблему в incompleteNote.
